...
CosCloud.prototype.getExpired = function(second){
	return Date.parse(new Date()) / 1000 + (second || 3600);
}
 ...
 * @param insertOnly
 */
CosCloud.prototype.out_uploadFile = function(success, error, bucketName, remotePath, file, insertOnly) {
	var fileSize = file.size;
	if (fileSize > 10 * 1024 * 1024) {
		this.sliceUploadFile(success, error, bucketName, remotePath, file, insertOnly, SLICE_SIZE_2M);
	} else {
		this.uploadFile(success, error, bucketName, remotePath, file, insertOnly);
	}
};
