...
class WalletController extends Controller
{
	...
	public function pre_wechatPay ()
	{
		...
		Utils::log($attributes['notify_url']);
	...
	public function wechatPay_hook ()
	{
		...
		Utils::logFrom("into wechat_hooks!", "wechatPay_hook.log");

		//        $wechat = app('wechat');
		$app = $this->get_app_wechat();

		Utils::log("into wechat_hooks!");

		$response = $app->payment->handleNotify(function($notify, $successful) {
			Utils::log("into response!");

			// 使用通知里的 "微信支付订单号" 或者 "商户订单号" 去自己的数据库找到订单
			Utils::log("order_id:" . $notify->transaction_id);

			//            $app_id = AppUtils::getAppID();
			Utils::log("order_id:" . $notify->out_trade_no);
	...
	{
		Utils::log("updateAccountStauts 1");

		$app_id = $this->getAppIdByOrderId($order_id);
		Utils::log("updateAccountStauts 2");
		...
		try {
			...
			if ($order_type == 2) {//购买成长型
				...
				//首页分类导航(即将上线)
				//会员(按时长收费)(即将上线)
				Utils::log("updateAccountStauts params 2");
...
gift_buy = 1,try_audio = 1,if_caption_define = 1,has_que=1,alive_by_video=1,daily_sign = 1,has_activity=1 where app_id = '$app_id'");
	...
	private function updateAccountAmount ($price, $order_id)
	{
		...
		if (!empty($resultWater)) {
			Utils::log("insert record success--app_id:" . $app_id);
			//更新客户余额
			...
			//            if($result_update){
			//                Utils::log("update account success");
			//
			//                \DB::commit();
			//                return true;
			//            }else{
			//                Utils::log("update account amount failed");
			//                \DB::rollBack();
			//                return false;
			//            }
			Utils::log("update account success");
			...
			Utils::log("insert record failed");
			...
			foreach ($order_list as $key => $order) {
				...
				if ($result_pay["return_code"] == "SUCCESS"
					...
					if ($result_pay["total_fee"] == $order_info->price) {
						...
						Utils::log("into opeator_pay_success!");
