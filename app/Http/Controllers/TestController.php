			...
			foreach ($answerer_list as $key => $answerer) {
				...
				if ($answerInfo) {
					...
					if ($result && property_exists($result, 'code') && $result->code === 0) {
						Utils::logFrom("用户:" . $answerer->answerer_id . ",模板消息通知发送成功!", 'fhw_question_list_' . date('Y-m-d', time()) . '.log');
					} else {
						Utils::logFrom("用户:" . $answerer->answerer_id . ",模板消息通知发送失败!", 'fhw_question_list_' . date('Y-m-d', time()) . '.log');
					}
		...
		foreach ($order_product_list as $key => $product) {
			...
			if ($product->resource_type == StringConstants::SINGLE_GOODS_PACKAGE || $product->resource_type == StringConstants::SINGLE_GOODS_MEMBER) {//专栏、会员
				...
				try {
					...
					Utils::logFrom('app_id:' . $product->app_id . ';product_id:' . $product->product_id, 'fhw_product_list.log');
				}
			} else {
				//获取资源的信息
		...
		foreach ($bannerList as $item) {
			...
			if (!empty($image_url)
				...
				try {
					...
					@unlink($src_image);
					@unlink($tar_image);
					Utils::logFrom($count . " " . $app_id . " " . $id . " " . $updateResult, "tempCompressBanner.log");
				} catch (\Exception $ex) {
					Utils::logFrom($count . " " . $app_id . " " . $id . " " . $image_url, "tempCompressBanner.log");
				}
			} else {
				Utils::logFrom($count . " " . $app_id . " " . $id . " not deal", "tempCompressBanner.log");
			}
		}
	}
		...
		foreach ($bannerList as $item) {
			...
			if (!empty($image_url)
				...
				try {
					...
					//                    @unlink($src_image);
					@unlink($tar_image);
					Utils::logFrom($count . " " . $app_id . " " . $id . " " . $updateResult, "tempCompressPro.log");
				} catch (\Exception $ex) {
					Utils::logFrom($count . " " . $app_id . " " . $id . " " . $image_url, "tempCompressPro.log");
				}
			} else {
				Utils::logFrom($count . " " . $app_id . " " . $id . " not deal", "tempCompressPro.log");
			}
		}
	}
...
");
		...
		foreach ($audio_list as $item) {
			...
			try {
				...
				Utils::logFrom($count . " " . $app_id . " " . $id . " Exception " . $ex->getMessage(), "tempCompressAudio.log");
			}
		}
	}
