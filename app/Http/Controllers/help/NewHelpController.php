...
class NewHelpController extends Controller
{
	...
	public function searchByContent ()
	{
		...
		$search_result = Searchy::search('db_help_document.t_help_article as t1')->fields('name')->select('t1.id', 't1.name', 't1.category_id as second_index', 't2.pid as first_index')->query($search_content)->getQuery()->join('db_help_document.t_help_category as t2', 't2.id', '=', 't1.category_id')->where('state', '<>', 2)->where('state', '!=', 1)->limit(50)->get();
