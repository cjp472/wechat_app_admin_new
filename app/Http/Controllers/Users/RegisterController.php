...
class RegisterController extends Controller
{
	...
	public function identifySubmit (Request $request)
	{
		...
		if ($merchant_id) {
			...
			Utils::logFrom("wechat sign:用户信息不存在!", "wechatSignError.log");
	...
	/*  // 生成所有 没有app_module记录 的用户的 t_app_module 记录
	  public static function supplyModule(){
		  ...
		  $count = count($id_arr);
		  Utils::logFrom("{$count}:共需插入t_app_module 数据 ","wechatSignError.log");
  //        dump($id_arr);
		  $i = 0;
		  $j = 0;
		  foreach ($id_arr as $v){
			  ...
			  if ($insert){
				  Utils::logFrom("{$i}:插入t_app_module 数据成功，app_id:{$v->app_id} ","wechatSignError.log");
			  } else {
				  Utils::logFrom("{$i}:插入t_app_module 数据失败，app_id:{$v->app_id} ","wechatSignError.log");
				  $j++;
			  }
		  }

		  Utils::logFrom("{$i}:累计插入t_app_module次 ","wechatSignError.log");
		  Utils::logFrom("{$j}:累计失败插入t_app_module 数据数 ","wechatSignError.log");
	  }*/
