...
@section('page_css')
    ...
    {{--弹窗--}}
    <link href="../css/external/jquery-alert.css?{{env('timestamp')}}" rel="stylesheet" type="text/css"/>
    {{--依赖end--}}
...
@section('ahead_js')
    {{--文本编辑器--}}
    <script src="../ueditor/ueditor.config.copy.js" type="text/javascript"></script>
...
@section('page_js')
    ...
    <script src="../js/admin/resManage/manageAudio.js?{{env('timestamp')}}" type="text/javascript"></script>
    <script type="text/javascript">
        ...
        function getTryDuration(element) {
            resAudio2Length = parseInt(element.duration);
            console.log(resAudio2Length);
        }
        golbalUrl =
        @if($page_type)
            '/edit_resource'
...
@section('base_mainContent')
    {{--dump(Illuminate\Support\Facades\Input::get('package_price', 0))--}}
    {{--dump($resource_info)--}}
        ...
        <div class="resAddPart resAddPart1">
          ...
          <div class="resAddSection">
                ...
                <div class="resAddSectionC">
                    <input class="resName inputDefault" name="resName" placeholder="请输入音频名称(建议字数在14字以内)"
                    @if($page_type)
            ...
            <div class="resAddSection">
                ...
                <div class="resAddSectionC">
                    ...
                    <div class="waves-effect btnSmall xeBtnDefault resUploadBtn">
                        选择文件
                        <input accept="audio/*" id="audio_url" class="upLoadAudio1" type="file">
                        </input>
                        <input type="hidden" id="Audio1Url" type="text"
                        @if($page_type)
                            value="{{$resource_info->audio_url}}"
                        @endif
                        />
                    </div>
                    @if($page_type && $resource_info->audio_url)
                    <div id="audioURLAudio1" class="hasUpNameContent">
                        <p class="hasUpName" title="{{$resource_info->file_name or $resource_info->audio_url}}">{{$resource_info->file_name or $resource_info->audio_url}}</p>
                        {{--<span>({{$resource_info->audio_size}})M</span>--}}
                    </div>
                    @endif
                    <div class="uploadBox uploadBoxAudio1" style="display: none">
                        <div class="uploadProgress">
                            ...
                            {{--<div class="uploadCancel uploadCancelA1">--}}
                                {{--取消--}}
                            {{--</div>--}}
                        </div>
                    </div>
            ...
            <div class="resAddSection">
                ...
                <div class="resAddSectionC">
                    ...
                    <div class="waves-effect btnSmall xeBtnDefault coverUpbtn">
                        ...
                        <input accept="image/jpeg,image/png,image/gif,image/bmp" class="upLoadImage upLoadImage1" type="file"/>
            ...
            <div class="resAddSection">
                ...
                <div class="resAddSectionC">
                    ...
                    <div class="waves-effect btnSmall xeBtnDefault resUploadBtn">
                        选择文件
                        <input accept="audio/*" class="upLoadAudio2" type="file">
                        </input>
                        <input type="hidden" id="Audio2Url" type="text"
                        @if($page_type)
                            value="{{$resource_info->try_audio_url}}"
                        @endif
                        />
                    </div>
                    @if($page_type && $resource_info->try_audio_url)
                    <div id="audioURLAudio2">
                        <span class="hasUpName" title="{{$resource_info->try_file_name or $resource_info->try_audio_url}}">{{$resource_info->try_file_name or $resource_info->try_audio_url}}</span>
                        <div class="deleteTryAudio">删除</div>
                    </div>
                    @endif
                    <div class="uploadBox uploadBoxAudio2" style="display: none">
                        <div class="uploadProgress">
                            <div class="uploadPTitle uploadAudio2Name"></div>
                            <div class="uploadPSize uploadAudio2Size">
                                (<span></span>M)
                            </div>
                            <div class="uploadPLine uploadPLineAudio2" style="display: none">
                                <div class="uploadPLineActive uploadPLineActiveAudio2">
                                </div>
                            </div>
                            <div class="uploadPersent uploadPersentAudio2">
                            </div>
                            {{--<div class="uploadCancel uploadCancelA2">--}}
                                {{--取消--}}
                            {{--</div>--}}
                        </div>
                        <div class="deleteTryAudio" style="display: none">删除</div>
                    </div>
                </div>
            </div>
            @endif
            ...
            <div class="resAddSection">
                ...
                <div class="resAddSectionC">
                    {{-- 图片预览，包括空的图片模板和图片预览模板 --}}
                    <div class="previewPicSign previewPicImage2">
                        <img src="@if($page_type&&$resource_info->sign_url) {{$resource_info->sign_url}} @else ../images/admin/resManage/pic_addriqian.png @endif">
                        </img>
                    </div>
                    <div class="waves-effect btnSmall xeBtnDefault coverUpbtn">
                        <span>
                        ...
                        <input accept="image/jpeg,image/png,image/gif,image/bmp" class="upLoadImage upLoadImage2"
                               type="file"/>
                        <input type="hidden" id="Image2Url" type="text"
                        @if($page_type)
                            value="{{$resource_info->sign_url}}"
                        @endif
                        />
                    </div>
                    <div class="coverUpTip">
                            ...
                            <div class="radioGroup" id="serviceToggle" data-setting="{{$is_set_temp}}" data-collection="{{$use_collection}}">
                                <div class="radioBtn1">
                                    ...
                                    <div class="upload_input_msg">
                                        <span>开启后，内容上架后会向已订阅专栏的用户发送服务号模板消息(每天最多可发送3次)。</span><a target="_blank" href="/helpCenter/problem?d_58f0bdbc5e504_LKZ3ouHf">什么是模板消息？</a>
                                    </div>
                                </div>
                                <div class="radioBtn2">
    ...
    <audio style="display:none;" id='tryTime' oncanplaythrough="getTryDuration(this)"></audio>
