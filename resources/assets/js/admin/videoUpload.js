...
$(document).ready(function () {
    ...
    $.cookie('content_create')? setTopUrlInfo('content_create') : setTopUrlInfo('video_listop');
                    ...
                    onOk: function(){
                        $.ajax('/is_industry',{
                            ...
                            if(code == 0){
                                ...
                                $().toastmessage('baseUtils.show.blueTip', "模板消息推送开启");
                            } else {
                                $().toastmessage('baseUtils.show.redTip', "无法开启消息推送，请按照提示修改设置");
                            }
                        }).fail(function(xhr, text, err) {
                            console.error(err);
                            $().toastmessage('baseUtils.show.redTip', "服务器出差啦，请稍后重试");
...
function changeSingleAndPackage() {
    $('.upload_type').on('click', function () {
        ...
        if ($('#single_select').hasClass('border_blue')) { //单卖
...
$(function () {
    ...
    $("#resource_pic").on("change", function () {
        ...
        if ($(this).attr("id") == 'resource_pic' && fileSize > 4 * 1000 * 1000) {
            baseUtils.show.redTip('封面图片太大!请不要超过4M!');
            return;
        }
    ...
    $("#patch_pic").on("change", function () {
        ...
        if ($(this).attr("id") == '#patch_pic' && fileSize > 4 * 1000 * 1000) {
            baseUtils.show.redTip('贴片图片太大!请不要超过4M!');
            return;
        }
...
function checkForm() {
    ...
    if (patch_pic.length == 0) {
        // alert('视频贴片图不能为空!');
        // return false;
        isEditPatchPic = false;
    } else {
        // var patch_pic_prop_pic = $('#patch_pic').prop('files');
        // var patch_pic_type = patch_pic_prop_pic[0].type;
        // if (patch_pic_type.indexOf('image') < 0) { // 'image/jpeg'
        //     alert('视频贴片图必须为图片文件!');
        //     return false;
        // }
        isEditPatchPic = true;
...
var patch_pic_errorCallBack = function (result) {
    //var jsonResult = $.parseJSON(result);
    hideLoading();
    baseUtils.show.redTip('贴片图上传失败!请重新上传!');
    uploadErrorShow(result.responseText);
};
