...
$(document).ready(function()
{
    ...
    if(reurlinfo)
    {
        if(reurlinfo.indexOf('feedback')>0){
            setTopUrlInfo('feedback_listop');
        }
        if(reurlinfo.indexOf('pay_admin')>0) {
            setTopUrlInfo('payadmin_listop');
        }
        if(reurlinfo.indexOf('comment')>0) {
            setTopUrlInfo('comment_listop');
        }
    }
    else{
        setTopUrlInfo('customer_listop');
    }
    ...
    $("#editButton").click(function()
    {
        ...
        window.location.href='/customeredit?appId='+appId+'&userId='+userId;
    });
    ...
    $("#customerMsg").click(function()
    {
        ...
        $(".modal-footer").children("button").eq(0).unbind("click").bind("click" , function ()
        {
            ...
            if (!$is_address_right) {
                window.wxc.xcConfirm('亲，外部链接输入有问题哦~','error');
                return false;
            }
