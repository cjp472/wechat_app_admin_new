...
var params={};
var roleParams={};//提交的角色参数集
var shaFlag=false;
var addIndex;//点击添加时所在的行
...
var url_app_id = "";
var hasSelectAlert,curSelected;
$(document).ready(function()
{
    ...
    hasSelectAlert = $('#push_ahead').val() != -1;
    ...
    $("#finish").click(function()
    {
        ...
        //选择收费形式
        var typeIndex=$(".blue").index();   //  （0，1，2， -> 专栏，单卖， 免费）
        if(typeIndex == 2)//免费
        {
        ...
        //直播详情封面
        if( $(".homePicShow").attr("src").length == undefined || $(".homePicShow").attr("src").length == 0)
        {
        ...
        //直播首页封面
        if( $(".picShow").attr("src").length == undefined || $(".picShow").attr("src").length == 0)
        {
        ...
        //  根据 alive_id + 价格 设置分销的开关

        var aliveId = $(".admin_data").data("alive_id");

        switch (typeIndex) {
            case 0: //  专栏
                if (checkedState == "unchecked") {
                    //  关闭分销设置
                    aliveUtils.closeSaleSet(aliveId);

                } else if (checkedState == "checked") {     //  专栏外单卖
...
function typeHandle()
{
    $(".typeSelect").click(function()
    {
        ...
        if(index == 2) //点击免费
        {
            ...
            //  处理所属分类
            if ($(".upload_item_div").hasClass("hide")) {
                $(".upload_item_div").removeClass("hide");
            }
...
{
    if( $("#alive_img_url").val().length!=0 )//修改了封面图
    {
        showLoading();
...
function uploadPic()
{
    if($("#img_url").val().length!=0)//修改了详情封面
    {
...
{
    if($("#videoName").html().trim().length!=0)//修改了视频
    {
        uploadVideo();
    }
    else
...
//云点播视频文件上传的真正执行过程,大文件SHA值计算期间不能上传
