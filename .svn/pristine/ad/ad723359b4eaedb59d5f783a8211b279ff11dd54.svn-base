...
@section('page_js')
    ...
    <script type="text/javascript" src="../js/admin/groupCode.js?{{env('timestamp')}}"></script>
@endsection
...
@section('base_mainContent')
    ...
    <div class="tab_div">
        ...
        @if($appmodule['group_buy'])<div class="tab_type tab_active" id="tab_group">团购</div>@endif
        ...
        <table class="table table-hover">
            ...
            <tbody>
                @foreach($data as $key => $value)
                    <tr>
                        <td>{{$value['id']}}</td>
                        <td><a href="/group_list?bid={{$value['id']}}">{{$value['name']}}</a></td>
