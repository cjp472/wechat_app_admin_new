...
function checkBannerForm() {
    ...
    if (resource_title.length != 0) {
        ...
        baseUtils.show.blueTip('图片名称不能为空！');
        $('#resouce_title').focus();
    }
    ...
    if (stop_time.length != 0) {
        if(start_time>stop_time){
            baseUtils.show.blueTip('上架时间不能晚于下架时间！');
            return false;
        }
...
//writeObj(allParams.params);
    ...
    $.post('/save_edit_banner', allParams, function (result) {
        ...
        var msg = result.msg;
        if (code == 0) {
            baseUtils.show.blueTip("更新成功!", function() {
            ...
            if(code ==1){
                window.wxc.xcConfirm(msg+',请重新选择！', 'error', {btn: parseInt("0001",2),onOk:function(){}});
            }
        }
    });
}
