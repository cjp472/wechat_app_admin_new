	...
	public static function createAppAllV4Folder ($app_id)
	{
		...
		foreach ($paths as $path) {
			...
			if ($result['code'] == 0 && $result['message'] = 'SUCCESS') {
				Utils::logFrom('SUCCESS appid=' . $app_id . " path=" . $path, 'createAppAllV4Folder.log');
			} else {
				Utils::logFrom('Failed appid=' . $app_id . " path=" . $path . " errMsg=" . print_r($result, 1), 'createAppAllV4Folder.log');
			}
		}
	}
