...
class Utils
{
	...
	public static function audioCompressing ($src_url, $bitrate = 64000)
	{
		...
		$tar_url            = $destination_folder . time() . '_' . basename($src_url);

		$cmd = 'ffmpeg -y -i ' . $src_url . ' -ab ' . $bitrate . ' -ar 44100 -ac 2 ' . $tar_url;
		Utils::log("cmd=" . $cmd);
	...
	{
		Utils::log("audioM3u8Compressing start");
		...
		exec($cmd . " 2>&1", $output, $status);
		Utils::log($output);
		Utils::log("audioM3u8Compressing end:" . $status);
...
| \xF4[\x80-\x8F][\x80-\xBF]{2}
	...
	public static function getAliveList ($m3u8_path)
	{
		...
		Utils::logFrom("GET M3U8 NEW" . $listNew, "getAliveList.log");
